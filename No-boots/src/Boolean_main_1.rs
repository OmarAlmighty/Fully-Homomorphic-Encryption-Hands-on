...
fn main() {
    ...
    let a_bits = [true, true, false, false, true, true, true, false,
         //true, true, false, false, true, true, true, false,
        //true, true, false, false, true, true, true, false,
                            ]; // 0b0011
    let b_bits = [true, false, true, false, true, true, true, false,
         //true, false, true, false, true, true, true, false,
        //true, false, true, false, true, true, true, false,
                            ]; // 0b0101

    let all_true: [bool; 8] = [false; 8];
    ...
    let ct_all_t: Vec<_> = all_true
        .iter()
        .map(|&bit| client_key.encrypt(bit))
        .collect();

    let start = Instant::now();
    let ct_product = xnor_gate(&server_key, &ct_a, &ct_b);

    let ct_product = xnor_gate(&server_key, &ct_product, &ct_all_t);

    let elapsed = start.elapsed();

    for (bit_a, bit_b) in a_bits.iter().zip(b_bits.iter()) {
        solution.push(!(!(bit_a ^ bit_b) ^ false) );
    }
