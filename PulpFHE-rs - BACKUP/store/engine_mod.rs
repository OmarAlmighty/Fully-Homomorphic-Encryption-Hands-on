...
pub(crate) trait BootstrapperEngine<C, K> {
    fn refresh_me(&mut self, ctxt: C, server_key: &K) -> Ciphertext;
}
...
impl BootstrapperEngine<&Ciphertext, ServerKey> for BooleanEngine {
    fn refresh_me(&mut self, ctxt: &Ciphertext, server_key: &ServerKey) -> Ciphertext {
        let buffer_lwe_before_pbs = convert_into_lwe_ciphertext_32(ctxt, server_key);
        let bootstrapper = &mut self.bootstrapper;
